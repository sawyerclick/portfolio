...
var svg = d3.select('#chart-04').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
var widthScale = d3.scaleLinear() // .domain([0, 100])
.range([0, width]);
var yPositionScale = d3.scaleBand().range([height, 0]).paddingInner(0.15);
d3.csv(require('../data/animal-counts-2.csv')).then(ready).catch(function (err) {
  console.log(err);
});
...
function ready(datapoints) {
  ...
  var countMax = d3.max(datapoints, function (d) {
    return +d.count;
  });
  widthScale.domain([0, countMax]);
  svg.selectAll('rect').data(datapoints).enter().append('rect').attr('fill', 'lightpink').attr('y', function (d) {
...
},{"d3":"../node_modules/d3/index.js","../data/animal-counts-2.csv":"data/animal-counts-2.csv"}]},{},["scripts/chart-04.js"], null)
