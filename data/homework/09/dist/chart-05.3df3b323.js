...
var width = 680 - margin.left - margin.right; // Add your svg
...
var parseTime = d3.timeParse('%B-%Y'); // Create your scales

var xPositionScale = d3.scaleLinear().range([0, width]);
var yPositionScale = d3.scaleLinear().range([height, 0]);
var fillScale = d3.scaleOrdinal().range(['#8dd3c7', '#ffffb3', '#bebada', '#fb8072', '#80b1d3', '#fdb462', '#b3de69', '#fccde5', '#d9d9d9', '#bc80bd']); // Create a d3.line function that uses your scales
...
function ready(datapoints) {
  ...
  svg.selectAll('path').data(nested).enter().append('path').attr('fill', 'none').attr('stroke', function (d) {
    return fillScale(d.key);
  }).attr('stroke-width', 2).attr('d', function (d) {
    return line(d.values);
  });
  svg.selectAll('circle').data(nested).enter().append('circle').attr('fill', function (d) {
    return fillScale(d.key);
  }).attr('r', 3).attr('cx', function (d) {
    var datapoints = d.values;
    var decData = datapoints[0];
  ...
  svg.selectAll('text').data(nested).enter().append('text').text(function (d) {
    return d.key;
  }).attr('fill', 'gray').attr('x', width).attr('dx', 10).attr('y', function (d) {
    var datapoints = d.values;
    var decData = datapoints[0];
    return yPositionScale(decData.price);
  }).attr('font-size', 12).attr('alignment-baseline', 'middle'); // Add your title

  svg.append('text').text('U.S. Housing Prices Fall in Winter').attr('x', width / 2).attr('dy', -10).attr('text-anchor', 'middle').attr('font-size', 22).attr('font-weight', 600).attr('fill', 'gray'); // Add the shaded rectangle

  svg.append('rect').attr('fill', 'lightgray').attr('x', xPositionScale(parseTime('December-16'))).attr('width', xPositionScale(parseTime('February-17')) - xPositionScale(parseTime('December-16'))).attr('y', 0).attr('height', height).lower(); // Add your axes
