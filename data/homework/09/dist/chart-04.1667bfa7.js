...
var svg = d3.select('#chart-04').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // scales

var xPositionScale = d3.scaleLinear().range([0, width]);
var yPositionScale = d3.scaleLinear().range([height, 0]); // Line
...
function ready(datapoints) {
  ...
  xPositionScale.domain([dateMin, dateMax]); // const pollutionMax = d3.max(datapoints, d => +d.Value)

  yPositionScale.domain([0, 500]); // Draw your areas
  ...
  svg.selectAll('circle').data(nested).enter().append('circle').attr('fill', highlight).attr('r', 3).attr('cx', function (d) {
  ...
  svg.selectAll('text').data(nested).enter().append('text').text(function (d) {
    return d.key;
  }).attr('fill', highlight).attr('x', width).attr('dx', 10).attr('y', function (d) {
    var datapoints = d.values;
    var decData = datapoints[datapoints.length - 1];
    return yPositionScale(decData.Value);
  }).attr('font-size', 12).attr('alignment-baseline', 'middle'); // Add your axes
