...
type Targets = string | NodeListOf<HTMLElement> | HTMLCollection | HTMLElement[];
 ...
 */
export default (node: HTMLElement, { targets }: KeythroughOptions = { targets: node.children }) => {
	// collect nodes
...
const getNodes = (node: HTMLElement, targets: Targets) => {
	...
	if (Array.isArray(targets)) nodes = targets; // array of nodes, as-is
		...
		if (dev) {
			console.warn(
				'[keythrough.ts]: `query` parameter must be a css selector string or a node list'
			);
		}
		nodes = undefined;
	}
