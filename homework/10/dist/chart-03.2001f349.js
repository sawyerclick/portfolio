...
var svg = d3.select('#chart-03').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
var widthScale = d3.scaleLinear().domain([0, 100]).range([0, width]);
var yPositionScale = d3.scaleBand().range([height, 0]).paddingInner(0.15);
d3.csv(require('../data/animal-counts.csv')).then(ready).catch(function (err) {
  console.log(err);
});
...
},{"d3":"../node_modules/d3/index.js","../data/animal-counts.csv":"data/animal-counts.csv"}]},{},["scripts/chart-03.js"], null)
