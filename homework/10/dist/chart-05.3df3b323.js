...
var svg = d3.select('#chart-05').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
var xPositionScale = d3.scaleLinear().domain([2001, 2007]).range([0, width]);
var yPositionScale = d3.scaleLinear().domain([0, 20]).range([height, 0]); // Line
...
d3.csv(require('../data/product_sales.csv')).then(ready).catch(function (err) {
  console.log(err);
});
...
function ready(datapoints) {
  ...
  console.log(nested);
  svg.selectAll('path').data(nested).enter().append('path').attr('fill', 'none').attr('stroke', highlight).attr('stroke-weight', 3).attr('d', function (d) {
...
},{"d3":"../node_modules/d3/index.js","../data/product_sales.csv":"data/product_sales.csv"}]},{},["scripts/chart-05.js"], null)
