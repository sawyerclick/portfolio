...
var svg = d3.select('#chart-06').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
...
var radiusScale = d3.scaleSqrt().range([1, 8]);
var colorScale = d3.scaleOrdinal().range(['#8dd3c7', '#fdb462', '#bebada', '#fb8072', '#80b1d3']);
d3.csv(require('../data/player_stats.csv')).then(ready).catch(function (err) {
  console.log(err);
});

function ready(datapoints) {
  var ppgMax = d3.max(datapoints, function (d) {
    return +d.PPG;
  });
  radiusScale.domain([0, ppgMax]); // colorScale.domain(datapoints.map(d => d.Pos))
  ...
  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('r', function (d) {
  ...
  }).attr('fill-opacity', 0.5).attr('class', function (d) {
    return d.Pos;
  }).attr('id', function (d) {
    if (d.PPG >= 20) {
      return 'highPPG';
    ...
    d3.select(this).attr('stroke', '#333333').attr('stroke-width', 1).raise();
    d3.select('#playerName').text('').append('text').text(d.Name);
    d3.select('#teamName').text('').append('text').text(d.Team);
  });
  d3.select('#c').on('click', function (d) {
    svg.selectAll('circle').attr('fill-opacity', 0.2).attr('fill', 'lightgray');
    svg.selectAll('.C').attr('fill-opacity', 0.6).attr('fill', function (d) {
      return colorScale(d.Pos);
    });
  });
  d3.select('#pg').on('click', function (d) {
    svg.selectAll('circle').attr('fill-opacity', 0.2).attr('fill', 'lightgray');
    svg.selectAll('.PG').attr('fill-opacity', 0.6).attr('fill', function (d) {
      return colorScale(d.Pos);
    });
  });
  d3.select('#ppg').on('click', function (d) {
    svg.selectAll('circle').attr('fill-opacity', 0.2).attr('fill', 'lightgray');
    svg.selectAll('#highPPG').attr('fill-opacity', 0.6).attr('fill', function (d) {
      return colorScale(d.Pos);
    }).raise();
  });
...
},{"d3":"../node_modules/d3/index.js","../data/player_stats.csv":"data/player_stats.csv"}]},{},["scripts/chart-06.js"], null)
