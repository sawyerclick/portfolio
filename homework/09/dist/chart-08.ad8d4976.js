...
var width = 250 - margin.left - margin.right; // And grabbing your container

var container = d3.select('#chart-08'); // Create your scales

var xPositionScale = d3.scaleLinear().range([0, width]);
var yPositionScale = d3.scaleLinear().range([height, 0]); // Create your area generator
...
var vColdTemp = '#236085';
var coldTemp = '#96bccf';
var normalTemp = '#cac7c7';
var hotTemp = '#ee9f71';
var vHotTemp = '#c9604b'; // Read in your data, then call ready

d3.tsv(require('../data/climate-data.tsv')).then(ready).catch(function (err) {
  console.log('Failed with', err);
}); // Write your ready function

function ready(datapoints) {
  var diff = datapoints.map(function (d) {
    return +d.diff;
  });
  ...
  yPositionScale.domain([0, d3.max(freq)]); // one function to rule them all
  ...
  function drawAllAreas(svg, startYear, endYear) {
    ...
    function drawTemp(svg, data, minTemp, maxTemp, color) {
      svg.append('path').datum(data).attr('d', line).attr('fill', color).attr('fill-opacity', 0.9).attr('stroke', 'none');
    }

    var vCold = nested.filter(function (d) {
      return d.key <= -3;
    });
    var cold = nested.filter(function (d) {
      return d.key <= -0.9 && d.key >= -3;
    });
    var normal = nested.filter(function (d) {
      return d.key >= -0.9 && d.key <= 0.9;
    });
    var hot = nested.filter(function (d) {
      return d.key <= 3 && d.key >= 0.9;
    });
    var vHot = nested.filter(function (d) {
      return d.key >= 3;
    });
    ...
    svg.append('text').text(startYear + ' to ' + endYear).attr('x', width / 2).attr('dy', -10).attr('text-anchor', 'middle').attr('font-size', 16).attr('font-weight', 600).attr('fill', 'gray');
    var xAxis = d3.axisBottom(xPositionScale).tickValues([-3, -0.9, 0.9, 3]).tickSize(-height);
    svg.append('g').attr('class', 'axis x-axis').attr('stroke-dasharray', '2 2').attr('transform', 'translate(0,' + height + ')').call(xAxis).selectAll('text').remove();
    ...
    function axisText(text, position, anchor, fill) {
      svg.append('text').text(text).attr('x', xPositionScale(position)).attr('y', height).attr('text-anchor', anchor).attr('fill', fill).attr('font-size', fontSize).attr('font-weight', fontWeight).attr('dy', fontOffset);
    }
  ...
  var data1951 = datapoints.filter(function (d) {
    return +d.year < 1981;
  });
  var nested1951 = d3.nest().key(function (d) {
    return d.diff;
  }).rollup(function (values) {
    return d3.mean(values, function (v) {
      return v.freq;
  ...
  function background1951(svg) {
    svg.append('path').datum(nested1951).attr('d', line).attr('fill', 'lightgray').attr('fill-opacity', 0.5).attr('stroke', 'none');
  } //  1951


  container.append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
...
},{"d3":"../node_modules/d3/index.js","../data/climate-data.tsv":"data/climate-data.tsv"}]},{},["scripts/chart-08.js"], null)
