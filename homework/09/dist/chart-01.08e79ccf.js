...
var svg = d3.select('#chart-01').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // Create a time parser

var parseTime = d3.timeParse('%Y-%m-%d'); // Create your scales

var xPositionScale = d3.scaleLinear().range([0, width]);
var yPositionScale = d3.scaleLinear().range([height, 0]); // Create a d3.line function that uses your scales
...
function ready(datapoints) {
  ...
  xPositionScale.domain([dateMin, dateMax]); // console.log("The x axis' min and max are:", dateMin, dateMax)

  var openMax = d3.max(datapoints, function (d) {
    return +d.Close;
  });
  var openMin = d3.min(datapoints, function (d) {
    return +d.Close;
  });
  yPositionScale.domain([openMin, openMax]); // console.log("The y axis' min and max are:", openMin, openMax)
  // Draw your dots

  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('fill', '#333333').attr('r', 3).attr('cx', function (d) {
    return xPositionScale(d.datetime);
  }).attr('cy', function (d) {
    return yPositionScale(d.Close);
  }); // Draw your single

  svg.append('path').datum(datapoints).attr('d', line).attr('stroke', '#333333').attr('stroke-width', 2).attr('fill', 'none'); // Add your axes

  var xAxis = d3.axisBottom(xPositionScale).tickFormat(d3.timeFormat('%m/%d/%y')).ticks(5);
  svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis);
  var yAxis = d3.axisLeft(yPositionScale);
  svg.append('g').attr('class', 'axis y-axis').call(yAxis);
}
},{"d3":"../node_modules/d3/index.js","../data/AAPL.csv":"data/AAPL.csv"}]},{},["scripts/chart-01.js"], null)
