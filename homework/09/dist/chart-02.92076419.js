...
var fillScale = d3.scaleOrdinal().range(['#8dd3c7', '#ffffb3', '#bebada', '#fb8072', '#80b1d3']); // Create a d3.line function
...
function ready(datapoints) {
  ...
  xPositionScale.domain([2000, dateMax]); // console.log("The x axis' min and max are:", dateMin, dateMax)

  var consumptionMax = d3.max(datapoints, function (d) {
    return +d.Value;
  }); // const consumptionMin = d3.min(datapoints, d => +d.Value)

  yPositionScale.domain([0, consumptionMax]); // console.log("The y axis' min and max are:", 0, consumptionMax)

  var nested = d3.nest().key(function (d) {
    return d.LOCATION;
  }).entries(datapoints); // Draw your dots

  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('r', 3).attr('fill', function (d) {
    return fillScale(d.LOCATION);
  }).attr('cx', function (d) {
    return xPositionScale(d.TIME);
  }).attr('cy', function (d) {
    return yPositionScale(d.Value);
