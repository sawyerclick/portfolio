...
function ready(datapoints) {
  ...
  container.selectAll('svg').data(nested).enter().append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
    ...
    var sumJP = d3.sum(datapoints.map(function (d) {
      return +d.ASFR_jp;
    })).toFixed(2);
    var sumUS = d3.sum(datapoints.map(function (d) {
      return +d.ASFR_us;
    })).toFixed(2);
    var usColor = '#2b83ba';
    var jpColor = '#d7191c'; // U.S.

    svg.append('path').datum(datapoints).attr('d', lineUS).attr('fill', usColor).attr('fill-opacity', 0.5).attr('stroke', 'none'); // japan

    svg.append('path').datum(datapoints).attr('d', lineJapan).attr('fill', jpColor).attr('fill-opacity', 0.4).attr('stroke', 'none');
    svg.append('text').text(sumUS).attr('x', 50).attr('dy', 15).attr('fill', usColor).attr('text-anchor', 'middle').attr('font-size', 10);
    svg.append('text').text(sumJP).attr('x', 50).attr('dy', 25).attr('fill', jpColor).attr('text-anchor', 'middle').attr('font-size', 10);
    svg.append('text').text(name).attr('x', width / 2).attr('dy', -10).attr('text-anchor', 'middle').attr('font-size', 16).attr('font-weight', 600).attr('fill', 'gray');
