...
var xPositionScale = d3.scaleLinear().domain([1980, 2010]).range([0, width]);
var yPositionScale = d3.scaleLinear().domain([0, 20000]).range([height, 0]); // Create your line generator
...
Promise.all([d3.csv(require('../data/middle-class-income.csv')), d3.csv(require('../data/middle-class-income-USA.csv'))]).then(ready).catch(function (err) {
  console.log('Failed with', err);
}); // Create your ready function

function ready(_ref) {
  var _ref2 = _slicedToArray(_ref, 2),
      datapointsAll = _ref2[0],
      datapointsUSA = _ref2[1];
  ...
  container.selectAll('svg').data(nested).enter().append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
    ...
    var name = d.key; // console.log(name)
    // const datapoints = d.values

    var usColor = 'gray';
    var otherColor = '#9e4b6c'; // U.S.

    svg.append('path').datum(datapointsUSA).attr('d', line).attr('fill', 'none').attr('stroke', usColor).attr('stroke-width', 2); // Other Countries

    svg.append('path').attr('fill', 'none').attr('stroke', otherColor).attr('stroke-width', 2).attr('d', function (d) {
      return line(d.values);
    });
    svg.append('text').text('USA').attr('x', width / 5).attr('dy', 20).attr('fill', 'gray').attr('text-anchor', 'middle').attr('font-size', 12).attr('font-weight', 500);
    svg.append('text').text(name).attr('x', width / 2).attr('dy', -10).attr('text-anchor', 'middle').attr('font-size', 16).attr('font-weight', 600).attr('fill', otherColor);
    ...
    var yAxis = d3.axisLeft(yPositionScale).tickValues([5000, 10000, 15000, 20000]).tickFormat(d3.format('$')).tickSize(-width);
    svg.append('g').attr('class', 'axis y-axis').call(yAxis);
    svg.selectAll('.tick line').attr('stroke-dasharray', '1 1').attr('stroke', 'lightgray');
    svg.selectAll('.domain').remove();
  });
}
},{"d3":"../node_modules/d3/index.js","../data/middle-class-income.csv":"data/middle-class-income.csv","../data/middle-class-income-USA.csv":"data/middle-class-income-USA.csv"}]},{},["scripts/chart-07.js"], null)
