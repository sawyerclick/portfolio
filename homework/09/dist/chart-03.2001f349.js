...
var yPositionScale = d3.scaleLinear().domain([0, 500]).range([height, 0]);
var fillScale = d3.scaleOrdinal().range(['#66c2a5', '#fc8d62', '#8da0cb']); // Do you need a d3.line function for this? Maybe something similar?
...
function ready(datapoints) {
  ...
  xPositionScale.domain([dateMin, dateMax]); // Draw your dots
  // svg
  //   .selectAll('circle')
  //   .data(datapoints)
  //   .enter()
  ...
  svg.selectAll('path').data(nested).enter().append('path').attr('fill', function (d) {
    return fillScale(d.key);
  }).attr('fill-opacity', 0.7).attr('stroke', 'none').attr('d', function (d) {
